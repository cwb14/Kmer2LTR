...
# comes from src/kmer2ltr/align.py exactly as a user would get it.
for f in arab_ltr_all_clean.fa.gz poa_ltr_all_clean.fa.gz human_ltr_all_clean.fa.gz \
  ...
  t0=$SECONDS
  $PY -m kmer2ltr "$D/$f" -o "$O/real_${b}.tsv" -t 20 -v
...
for n in negatives shuffled_truth; do
  echo "=== $n ($(date)) ==="
  $PY -m kmer2ltr "bench/out/${n}.fa" -o "$O/neg_${n}.tsv" -t 20 -v
done

# Final runs of both benchmark grids at the shipped defaults.
$PY -m kmer2ltr bench/out/homology_grid.fa -o "$O/final_homology.tsv" -t 20 -v
$PY bench/homology_grid.py score --truth bench/out/homology_truth.tsv \
    --pred "$O/final_homology.tsv" --out "$O/cells_hom_FINAL.json"
$PY -m kmer2ltr bench/out/gold_perturbed.fa -o "$O/final_gold.tsv" -t 20 -v
$PY -c "
